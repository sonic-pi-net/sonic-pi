...
change the speed the numbers are fed to the speaker to change the sound.

## Changing Rate

Let's play with one of the ambient sounds: `:ambi_choir`. So to play it
...
However by changing rate we can switch up genres. Try half speed for
*old school hip-hop*:
...
frequency or pitch of a wave form is determined by how fast it moves up
...
frequency you hear it*. Also, *halving the rate will half the
