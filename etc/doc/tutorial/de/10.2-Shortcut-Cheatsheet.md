...
* `M-Z` bedeutet, die *Meta*-Taste zu drücken und zu halten, dann *Shift* zu drücken und zu halten und schließlich die *z*-Taste und alle drei Tasten gleichzeitig wieder los zu lassen.
...
* `M-+` - Schrift vergrößern
* `M--` - Schrift verkleinern
* `M-m` - Code ausrichten
* `C-g` - Escape
