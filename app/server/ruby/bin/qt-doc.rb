...
# Documented opts of plain lang functions (live_audio's input:, live_loop's
# sync:, …): the generic opt-completion branch offers them (see the fn-opts
# table below), so give each key a summary/doc for the popup's helper pane.
# First writer wins on name collisions, matching opt_summaries.
docs << "\n  // lang-function opt docs (for the generic opt completion branch)\n"
lang_opt_docs = {}
fn_info.each do |_, info|
  opts = info[:opts]
  next unless opts.is_a?(Hash)
  next if opts.key?(:your_key)   # cue-style placeholders: arbitrary user keys
...
# Fn-opts table: plain lang function -> its documented opt keys, from the `opts:`
# maps in the lang docs. Feeds the generic opt-completion branch (consumed by
# SonicPi::resolveFnOpts; see completion_context.h) and is pinned by
# gui-tests/completion_fnopts.test.cpp. cue-style placeholder opts (your_key: …)
# document arbitrary user keys, not completable vocabulary — skipped.
fnopts_by_name = {}
argkind_mods.each do |mod|
  next unless mod.respond_to?(:docs)
  mod.docs.each do |name, info|
    ...
    fnopts_by_name[name.to_s] ||= opts.keys.map { |k| "#{k}:" }
  end
end
fnopts_entries = fnopts_by_name.map do |name, keys|
  "    t.insert(\"#{name}\", QStringList{ #{keys.map { |k| "\"#{k}\"" }.join(', ')} });"
end

fnopts_h = []
fnopts_h << "// AUTO-GENERATED by qt-doc.rb — do not edit.\n"
fnopts_h << "#ifndef SONICPI_COMPLETION_FNOPTS_GEN_H\n"
fnopts_h << "#define SONICPI_COMPLETION_FNOPTS_GEN_H\n\n"
fnopts_h << "#include \"completion_context.h\"\n\n"
fnopts_h << "namespace SonicPi {\n\n"
fnopts_h << "inline FnOptsTable generatedFnOpts()\n{\n    FnOptsTable t;\n"
fnopts_h << fnopts_entries.sort.join("\n") << "\n"
fnopts_h << "    return t;\n}\n\n} // namespace SonicPi\n\n#endif\n"
