...
def generate_section(lang, section, toc)
  toc.each do |section, sec_list|
    sec_list["pages"].each do |page_info|
      generate_page_and_subpages(page_info, lang, section)
    end
  end
end

def generate_page_and_subpages(page_info, lang, section)
  puts page_info["path"]

  html = ""
  File.open("#{etc_path}/doc/generated_html/#{lang}/#{page_info["path"]}", 'r:UTF-8') do |f|
    html = f.read()
  end

  File.open("#{etc_path}/doc/html/generated_src/#{lang}/#{page_info["path"]}", 'w') do |f|
    f << "---\n"
    f << "layout: doc_manual\n"
    f << "title: \"#{page_info["name"].gsub(/"/, '\\"')}\"\n"
    if page_info.has_key?("keyword") then
      f << "\"#{page_info["keyword"].downcase}\""
    end
    f << "lang: #{lang}\n"
    f << "section: #{section}\n"
    f << "---\n"
    f << change_img_links_for_website(html) << "\n"
  end

  if page_info.has_key?("subpages")
    page_info["subpages"].each do |subpage_info|
      generate_page_and_subpages(subpage_info, lang, section)
    end
  end
end
...
languages.each do |lang|
  ...
  # Generate pages with front matter
  ["tutorial", "synths", "fx", "samples", "lang"].each do |section|
    generate_section(lang, section, toc)
  end

  # Append info pages to table of contents
  info_toc = {
    :pages => [
    ]
  }
...
# Write the version number (for the sidebar)
