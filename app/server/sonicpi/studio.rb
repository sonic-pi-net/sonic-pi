...
module SonicPi
  class Studio
    ...
    def start_mixer
      ...
      @mixer = @server.trigger_synth(:head, @mixer_group, "mixer", {"in_bus" => @mixer_bus})
    end
    ...
    def recording_start(path, bus=0)
      ...
      @recording_mutex.synchronize do
        ...
        s = @server.trigger_synth :head, @recording_group, "recorder", {"out-buf" => bs.to_i, "in_bus" => bus}, true
        @recorders[bus] = [bs, s]
        true
      end
    end
