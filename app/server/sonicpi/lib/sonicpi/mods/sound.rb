       ...
       def play(n, *args)
         ...
         if tuning_system = Thread.current.thread_variable_get(:sonic_pi_mod_sound_tuning)
           if tuning_system != :equal
             ...
             @@tuned_notes ||= {}
             @@tuned_notes[tuning_system] ||= (0..150).to_a.map {|midi_note|
             ...
             if n.is_a? Float
               note_above = @@tuned_notes[tuning_system][n.ceil]
               note_below = @@tuned_notes[tuning_system][n.floor]
               n_diff = n - n.floor

               n = hz_to_midi(midi_to_hz(note_below) + ((midi_to_hz(note_above) - midi_to_hz(note_below)) * n_diff))
             else
