# racy: native plays a sample from the thread that loaded it when the loader has not finished, and from the
# calling thread when it has — two traces for one program, on its real threads. The runtime is deterministic
# here and gets one attempt; the oracle is run again before a failure is believed (scripts/check.rb).
# a real track: DJ_Dave's pattern functions (string rings) and fx whose opts tick (a do block after a parenthesised call)
# horizon: 4
# Coded by DJ_Dave
use_bpm 120   # at twice the tempo: specs/lang/real_track_dj_dave.rb
